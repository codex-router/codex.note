...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate code insight from C/C++/Java/Go/Python/Bash/JavaScript/TypeScript/Rust repos using AI prompts.",
    )
    parser.add_argument("--repo", required=True, help="Path to target source repository")
    parser.add_argument("--out", required=True, help="Path to generated insight directory")
    parser.add_argument(
        "--include",
        action="append",
        default=[],
...
def generate_module_insight(module: str, analysis_json: dict, insight_prompt: str) -> str:
...
def build_system_page(module_pages: list[str]) -> str:
    ...
    lines.append("")
    lines.append("## Generation Insights")
    lines.append("")
    lines.append("- Generated by `generate_insight.py` in two-pass mode (analysis -> insight).")
...
def main() -> int:
    ...
    state_dir = Path(__file__).resolve().parent / ".codex-insight" / "analysis"
    ...
    insight_prompt = read_prompt_template("insight_prompt.txt")
    ...
    for module, snippets in sorted(modules.items()):
        ...
        print(f"[2/2] Generating insight page: {module}")
        markdown = generate_module_insight(module, analysis, insight_prompt)
    ...
    print(f"Done. insight written to: {out}")
    return 0
